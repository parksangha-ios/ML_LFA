...
N_L     = 20   # log-L 방향 등분
N_TIME  = 50   # 그래프 당 포인트 수 (최적 포인트 수)
...
# -----------------------------------
# 3. 데이터 생성 루프 (Long Format, ideal + heatloss)
# -----------------------------------
...
for log_alpha in alpha_log_grid:
    ...
    for log_L in L_log_grid:
        ...
        for _ in range(N_IDEAL_PER):
            ...
            L_vals      = np.full(N_TIME, log_L, dtype=np.float64)
            alpha_vals  = np.full(N_TIME, log_alpha, dtype=np.float64)
            loss_vals   = np.full(N_TIME, 0.0, dtype=np.float64)  # ideal: loss_factor = 0
...
save_path = "C:/Users/Heum/Desktop/Sang Ha Park/CODE/ML_LFA/lfa_mixed_ideal_heatloss_50pt.csv"
